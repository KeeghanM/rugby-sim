...
import type { Career, InboxMessage, Player, ScoutingReport } from './types.ts'
...
export function calculatePlayerMarketValue(player: Player): number {
  ...
  const base = (Math.max(45, ovr) / 50) ** 4 * 22_000
  const value = Math.round((base * ageMultiplier) / 1000) * 1000
  return Math.max(10_000, value)
}

export function generateScoutingReport(player: Player, scoutLevel: number, currentRound: number): ScoutingReport {
  const ovr = getPlayerOverall(player)
  const potential = player.potential ?? Math.min(99, ovr + Math.max(0, 30 - player.age))
  ...
  if (player.strength >= 75) strengths.push('Dominant physical power & collision impact')
  else if (player.strength < 55) weaknesses.push('Vulnerable in heavy contact')

  if (player.skills.tackling >= 75) strengths.push('Rock-solid defensive tackle technique')
  else if (player.skills.tackling < 55) weaknesses.push('Prone to missed tackles in open play')

  if (player.skills.handling >= 75) strengths.push('Silky ball handling and offloading')
  else if (player.skills.handling < 55) weaknesses.push('Susceptible to handling errors in traffic')

  if (strengths.length === 0) strengths.push('Balanced all-round fundamental skillset')
  if (weaknesses.length === 0) weaknesses.push('No glaring technical deficiencies')
...
export function scoutTargetPlayer(career: Career, playerId: string): Career {
  ...
  const report = generateScoutingReport(foundPlayer, scoutLevel, career.currentRound)
...
export function signFreeAgent(career: Career, playerId: string, offeredWage: number, signingBonus: number): Career {
  ...
  if (managedClub.squad.length >= 40) {
    throw new Error('Squad limit reached (40 players). Release a player before signing free agents.')
  }
...
export function releaseSquadPlayer(career: Career, playerId: string): Career {
  ...
  if (managedClub.squad.length <= 23) {
    throw new Error('Cannot release player: club must maintain at least 23 players for matchday team sheets.')
  }
...
export function submitTransferBid(
  ...
  const buyerClub = career.season.clubs.find((c) => c.id === career.managedClubId)
  ...
  if (buyerClub.squad.length >= 40) {
    throw new Error('Squad limit reached (40 players). Release a player before making transfer bids.')
  }
  ...
  const minRequiredFee = isStarter ? Math.round(marketVal * 1.25) : Math.round(marketVal * 0.95)
...
export function respondToIncomingTransferBid(career: Career, offerId: string, decision: 'accept' | 'reject'): Career {
  ...
  if (!sellerClub || !buyerClub) throw new Error('Club involved in transfer not found')
  ...
  const updatedOffers = career.transferOffers.map((o) =>
    o.id === offerId ? { ...o, status: decision as 'accepted' | 'rejected' } : o,
  )
