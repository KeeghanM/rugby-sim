import type { MatchInput, PlayerSkills, TeamDefinition } from '../../simulation/domain.ts'
...
export function clubToTeamDefinition(club: Club, manager?: ManagerProfile): TeamDefinition {
  ...
  const defaultSkills: PlayerSkills = {
    decision: Math.min(0.98, 0.55 + (club.reputation / 100) * 0.35 + disciplineBonus),
  ...
  club.squad.forEach((player, index) => {
    ...
    playerOverrides[jerseyNumber] = {
      ...
      skills: {
        decision: Math.max(0.1, Math.min(0.99, player.skills.decision / 100 + disciplineBonus)),
...
export function createMatchInputForFixture(career: Career, fixture: Fixture): MatchInput {
