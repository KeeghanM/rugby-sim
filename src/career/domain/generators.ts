import { CLUBS, FIRST_NAMES, LAST_NAMES, PLAYER_ROLES, ROLE_GROUPS, STAFF_ROLES, type PlayerRole } from './constants.ts'
...
import type { Career, Club, Fixture, Player, PlayerCareerRecord, StaffMember } from './types.ts'
...
export function generatePlayerStats(role: PlayerRole, playerIndex: number, clubIndex: number) {
    ...
    return {
      ...
      squad: PLAYER_ROLES.map((role, playerIndex) => {
        ...
        const wage = Math.round(
          850 + (playerIndex < 15 ? 900 : playerIndex < 23 ? 500 : 250) + (stats.strength + stats.speed) * 6,
        )
        const age = 19 + ((playerIndex * 7 + clubIndex * 3) % 17)
        const ovrEst = (stats.strength + stats.speed + stats.skills.tackling) / 3
        const potential = Math.min(99, Math.round(ovrEst + Math.max(0, 31 - age) * 0.8))
      ...
      staff: staffMembers,
      staffLevel: Math.round(staffMembers.reduce((sum, s) => sum + s.level, 0) / staffMembers.length),
...
export function createFixtures(): Fixture[] {
  ...
  for (let round = 1; round <= CLUBS.length - 1; round += 1) {
    for (let pair = 0; pair < CLUBS.length / 2; pair += 1) {
      ...
      if (left === undefined || right === undefined) throw new Error('Invalid fixture rotation')
...
export function createFreeAgents(count = 24): Player[] {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    const ovrEst = (stats.strength + stats.speed + stats.skills.tackling) / 3
    const potential = Math.min(99, Math.round(ovrEst + Math.max(0, 31 - age) * 0.9))
...
export function createCareer(managerName: string, clubId: string): Career {
  ...
  if (!CLUBS.some((club) => club.id === clubId)) throw new Error(`Unknown club: ${clubId}`)
