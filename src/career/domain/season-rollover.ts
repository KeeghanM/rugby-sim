import { SEASON_PRIZE_MONEY } from './constants.ts'
import { generateYouthIntake } from './academy.ts'
import { createFixtures, mondayForRound } from './generators.ts'
...
import type { Career, Club, InboxMessage, Player, SeasonArchive } from './types.ts'
...
export function processPlayerYearProgression(
  ...
  if (nextAge >= 33) {
    const retireChance = nextAge >= 36 ? 0.85 : nextAge === 35 ? 0.55 : nextAge === 34 ? 0.3 : 0.15
...
export function executeSeasonRollover(career: Career): Career {
  ...
  const _isUserChampion = champion?.clubId === career.managedClubId

  const userStandingIndex = standings.findIndex((s) => s.clubId === career.managedClubId)
  ...
  const updatedClubs: Club[] = career.season.clubs.map((club) => {
    ...
    const progressedAcademy = club.academySquad.map((p) => {
      const { player: progressed } = processPlayerYearProgression(p, clubFacilityLvl, academyLvl)
      return progressed
    })

    // If senior squad has vacancies below 35, promote top academy players
    const currentSenior = [...remainingSeniorSquad]
