...
export function getManagerLevel(xp: number): {
  ...
  const nextLevelFloor =
    LEVEL_THRESHOLDS[level] ?? currentLevelFloor + level * 500;
  const diff = nextLevelFloor - currentLevelFloor;
  const progress =
    diff > 0 ? Math.min(1, Math.max(0, (xp - currentLevelFloor) / diff)) : 1;
...
export function isTacticsUnlocked(
...
export function getManagerPerks(manager: ManagerProfile) {
  ...
  for (const qId of manager.qualifications) {
    const course = COACHING_COURSES[qId];
    if (!course) continue;
    if (course.unlocks.trainingBonusPct)
      trainingBonusPct += course.unlocks.trainingBonusPct;
    if (course.unlocks.matchXpBonusPct)
      matchXpBonusPct += course.unlocks.matchXpBonusPct;
    if (course.unlocks.disciplineBonus)
      disciplineBonus += course.unlocks.disciplineBonus;
  }
...
export function enrollCoachingCourse(
  career: Career,
  ...
  if (career.manager.activeCourse) {
    throw new Error(
      "You are already enrolled in a coaching course. Complete it before starting another.",
    );
  }
  if (career.manager.level < course.levelRequired) {
    throw new Error(
      `Requires Manager Level ${course.levelRequired} (Current Level: ${career.manager.level})`,
    );
  }
...
export function updatePlaybookTactics(
  career: Career,
  playbookUpdates: Partial<PlaybookTactics>,
): Career {
  ...
  const next: PlaybookTactics = {
    ...
    defenseStructure:
      playbookUpdates.defenseStructure ?? current.defenseStructure,
  ...
  if (
    !isTacticsUnlocked(
      career.manager,
      "attackStructure",
      next.attackStructure,
    ) ||
    !isTacticsUnlocked(
      career.manager,
      "defenseStructure",
      next.defenseStructure,
    ) ||
    !isTacticsUnlocked(career.manager, "setPieceFocus", next.setPieceFocus)
  ) {
    throw new Error(
      "Selected tactic is locked. Complete the required coaching qualification first.",
    );
  }
