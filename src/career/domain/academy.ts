...
export function generateYouthIntake(
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    const potential = Math.min(
      99,
      Math.max(72, Math.round(ovrEst + 15 + potentialBonus + (i % 7))),
    );
...
export function promoteAcademyProspect(
  career: Career,
  prospectId: string,
): Career {
  ...
  if (managedClub.squad.length >= 40) {
    throw new Error(
      "Senior squad is full (40 players max). Release or sell a senior player before promoting youth.",
    );
  }
...
export function dismissAcademyProspect(
  career: Career,
  prospectId: string,
): Career {
  const updatedClubs = career.season.clubs.map((club) => {
