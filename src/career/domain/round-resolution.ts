...
export function resolveRound(
  ...
  const updatedClubsMap = new Map<string, Club>(career.season.clubs.map((c) => [c.id, { ...c, squad: [...c.squad] }]))

  const fixtures = career.season.fixtures.map((fixture) => {
    if (fixture.round !== career.currentRound || fixture.status === 'played') return fixture
    const recorded = recordedResults?.get(fixture.id)
    let result: MatchResult

    const input = createMatchInputForFixture(career, fixture)
    const simResult = recorded?.resultObj ?? simulateMatch({ input, seed: fixture.seed })

    const homeClub = updatedClubsMap.get(fixture.homeClubId)
    const awayClub = updatedClubsMap.get(fixture.awayClubId)

    const fixturePlayers: FixturePlayerPerformance[] = simResult.players.map((p) => {
      const club = p.team === 0 ? homeClub : awayClub
      const playerObj = club?.squad.find((pl) => pl.id === p.playerId)
      return {
        playerId: p.playerId,
        clubId: club?.id ?? (p.team === 0 ? fixture.homeClubId : fixture.awayClubId),
    ...
    for (const clubId of [fixture.homeClubId, fixture.awayClubId]) {
      ...
      club.squad = club.squad.map((player, index) => {
        const matchPerf = simResult.players.find((p) => p.playerId === player.id)
        ...
        if (matchPerf) {
          ...
          updatedCareerRecord = {
            ...
            subAppearances: updatedCareerRecord.subAppearances + (matchPerf.started ? 0 : 1),
            ...
            distanceCovered: updatedCareerRecord.distanceCovered + st.distanceCovered,
            distanceCarried: updatedCareerRecord.distanceCarried + st.distanceCarried,
            successfulPasses: updatedCareerRecord.successfulPasses + st.successfulPasses,
            totalPasses: updatedCareerRecord.totalPasses + st.totalPasses,
            successfulKicks: updatedCareerRecord.successfulKicks + st.successfulKicks,
            totalKicks: updatedCareerRecord.totalKicks + st.totalKicks,
            penaltiesConceded: updatedCareerRecord.penaltiesConceded + st.penaltiesConceded,
        ...
        if (player.injury === null && playedInMatch) {
          ...
          matchInjuryChance = Math.max(0.005, matchInjuryChance - medLevel * 0.006)
