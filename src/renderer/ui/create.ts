...
export const createUI = (state: GameState, onExit?: () => void) => {
  ...
  const managerCloseBtn = requiredElement('manager-close-btn', HTMLButtonElement)
  ...
  const managerTeamSummary = requiredElement('manager-team-summary', HTMLDivElement)
  const managerRosterThead = requiredElement('manager-roster-thead', HTMLTableSectionElement)
  const managerRosterTbody = requiredElement('manager-roster-tbody', HTMLTableSectionElement)
  ...
  const updateSpeedDisplay = () => {
    speedDisplay.textContent = simulationSpeed === 0 ? '0.0× (Paused)' : `${simulationSpeed.toFixed(1)}×`
  }
