...
export const syncUI = (game: GameState, ctx: UIContext, scene: Scene, engine: Engine) => {
  ...
  if (p.kind === 'openPlay') phaseDesc = 'Open play'
    ...
    phaseDesc = p.reason === 'goalLineDropout' ? `Goal-line dropout ${p.stage}` : `Kickoff ${p.stage}`
  ...
  if (tvTeam0) {
    ...
    tvTeam0.style.borderBottomColor = isPoss ? game.teams[0].color : 'transparent'
    tvTeam0.style.background = isPoss ? `${game.teams[0].color}33` : 'transparent'
  ...
  if (tvTeam1) {
    ...
    tvTeam1.style.borderBottomColor = isPoss ? game.teams[1].color : 'transparent'
    tvTeam1.style.background = isPoss ? `${game.teams[1].color}33` : 'transparent'
  ...
  const isOpenPlayOrRuck = p.kind === 'openPlay' || p.kind === 'ruck' || p.kind === 'maul'
  ...
  if (tvShotClock) {
    ...
    if (showShotClock) tvShotClock.textContent = `SHOT ${Math.max(0, Math.ceil(30 - p.elapsed * 6))}`
  }
  if (scoreboard) {
    if (ctx.isDebugMode()) {
  ...
  if (ctx.manager.shouldRender(performance.now())) {
    syncManager(game, ctx, ctx.manager.getSelectedTeam(), ctx.manager.getSelectedView())
  }
