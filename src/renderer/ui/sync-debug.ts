...
export const syncDebug = (game: GameState, ctx: UIContext, scene: Scene, engine: Engine) => {
  ...
  if (ballCard) {
    ...
    if (
      ...
      const carrier = game.players.find((player) => player.id === game.ball.carrierId)
      ...
      if (game.ball.intendedReceiverId) {
        const receiver = game.players.find((player) => player.id === game.ball.intendedReceiverId)
        targetText = receiver ? `Receiver #${receiver.number} ${receiver.role}` : game.ball.intendedReceiverId
      ...
      const lastTouch =
        game.ball.lastTouchedTeam !== null ? escapeHtml(game.teams[game.ball.lastTouchedTeam].name) : 'None'
