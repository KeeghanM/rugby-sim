...
import { PITCH } from '../simulation/domain.ts'
...
const createGoalPosts = (scene: Scene) => {
  ...
  const buildPostSet = (name: string, z: number) => {
    const leftUpright = CreateCylinder(`${name}-left`, { diameter: 0.18, height: 14 }, scene)
    leftUpright.position.set(-2.8, 7, z)
    leftUpright.material = postMat

    const rightUpright = CreateCylinder(`${name}-right`, { diameter: 0.18, height: 14 }, scene)
    rightUpright.position.set(2.8, 7, z)
    rightUpright.material = postMat

    const crossbar = CreateCylinder(`${name}-crossbar`, { diameter: 0.16, height: 5.6 }, scene)
    crossbar.rotation.z = Math.PI / 2
    crossbar.position.set(0, 3, z)
    crossbar.material = postMat

    const leftPad = CreateCylinder(`${name}-left-pad`, { diameter: 0.55, height: 1.8 }, scene)
    leftPad.position.set(-2.8, 0.9, z)
    leftPad.material = padMat

    const rightPad = CreateCylinder(`${name}-right-pad`, { diameter: 0.55, height: 1.8 }, scene)
...
export const createPitch = (scene: Scene) => {
  const ground = CreateGround('ground', { width: PITCH.width, height: PITCH.totalLength }, scene)
  ...
  const across = (name: string, z: number, dashed = false) =>
    mark(name, new Vector3(PITCH.touchLines.left, 0.02, z), new Vector3(PITCH.touchLines.right, 0.02, z), dashed)
  const along = (name: string, x: number, dashed = false) =>
    mark(name, new Vector3(x, 0.02, PITCH.tryLines.south), new Vector3(x, 0.02, PITCH.tryLines.north), dashed)
