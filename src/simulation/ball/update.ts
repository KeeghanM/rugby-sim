import { attackDirection, type GameState, otherTeam, PITCH, type Player, type Team } from '../domain.ts'
...
const throwingTeamAtTouch = (state: GameState): Team => {
  ...
  const oppositionTwentyTwo = direction === 1 ? PITCH.twentyTwoMetreLines.north : PITCH.twentyTwoMetreLines.south
  ...
  return state.pendingLineoutTeam !== null ? state.pendingLineoutTeam : is5022 ? kickerTeam : otherTeam(kickerTeam)
}
...
export const updateBall = (state: GameState, deltaSeconds: number, random: Random) => {
  ...
  if (!state.ball.flight) {
    if (state.phase.kind === 'openPlay' && Math.abs(state.ball.position.z) >= Math.abs(PITCH.deadBallLines.north)) {
    ...
    if (picker) {
      attemptPossession(state, picker, (1 - effectiveSkill(picker, 'handling')) * 0.08, random)
    }
    return
  }
  ...
  if (state.ball.flight === 'rolling') {
    ...
    if (state.phase.kind !== 'lineout' && Math.abs(state.ball.position.x) >= PITCH.touchLines.right) {
      startLineout(
        state,
    ...
    const rollingPicker = state.players
      ...
      .sort((a, b) => distance(a.position, state.ball.position) - distance(b.position, state.ball.position))[0]
    if (rollingPicker) {
      attemptPossession(state, rollingPicker, (1 - effectiveSkill(rollingPicker, 'handling')) * 0.1, random)
      return
    }
  ...
  if (state.ball.flight === 'dropGoal') {
    ...
    const targetTryLine = dir === 1 ? PITCH.tryLines.north : PITCH.tryLines.south
  ...
  const isGoalAttempt =
    (state.phase.kind === 'conversion' || (state.phase.kind === 'penalty' && state.phase.choice === 'goal')) &&
  ...
  if (
    state.phase.kind === 'openPlay' &&
    (state.ball.flight === 'kick' || state.ball.flight === 'kickoff' || state.ball.flight === 'dropGoal') &&
    Math.abs(state.ball.position.x) < PITCH.touchLines.right &&
    Math.abs(state.ball.position.z) >= Math.abs(PITCH.deadBallLines.north)
  ) {
    startGoalLineDropout(state, state.ball.position.z)
  ...
  if (catchable && state.ball.position.y <= 2.2) {
    ...
    if (catcher) {
      attemptPossession(state, catcher, 0.01 + (1 - effectiveSkill(catcher, 'handling')) * 0.18, random, 0.15)
      return
    }
  }
  if (state.ball.position.y <= 0.15) {
    ...
    const isHighKick = flight === 'kick' || flight === 'kickoff' || flight === 'dropGoal'
    ...
    if (isHighKick && state.ball.bouncesRemaining > 0) {
      ...
      state.ball.velocity.y = Math.abs(state.ball.velocity.y) * (0.35 + random() * 0.25)
      ...
      if (bounceType < 0.25) {
        ...
        state.ball.velocity.z = -state.ball.velocity.z * (0.35 + random() * 0.45)
