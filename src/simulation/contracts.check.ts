...
const input = createMatchInput(teams, {
  0: {
    ...
    substitutes: Array.from({ length: 8 }, (_, index) => `club-a-${index + 16}`),
  },
  1: {
    starters: Array.from({ length: 15 }, (_, index) => `club-b-${index + 1}`),
    substitutes: Array.from({ length: 8 }, (_, index) => `club-b-${index + 16}`),
  },
})
...
check(new Set(result.players.map((player) => player.playerId)).size === 46, 'Result player IDs must be unique')
...
if (kickoff.kind !== 'kickoff') throw new Error('Match did not start at kickoff')
...
check(kickoff.stage === 'forming', 'Kickoff started before every player formed')
kickoffState.players[0].position = getKickoffTarget(
  ...
  kickoffState.activeShapePositions[kickoffState.players[0].team][
    kickoffState.players[0].team === kickoff.kickingTeam ? 'kickoffAttack' : 'kickoffDefence'
  ],
)
...
if (penaltyState.phase.kind !== 'penalty') throw new Error('Penalty did not start')
...
updatePenalty(penaltyState, 0.1, () => 0.5)
check(
  penaltyState.ball.kickOrigin?.x === 12 && penaltyState.ball.kickOrigin.z === 18,
...
check(
  penaltyState.players.every(({ ruckRecoverySeconds }) => ruckRecoverySeconds === 0),
  'Contact pose survived lineout transition',
)
check(
  separatedVelocity(penaltyState, penaltyState.players[0], { x: 1, z: 2 }).x === 1 &&
    separatedVelocity(penaltyState, penaltyState.players[0], { x: 1, z: 2 }).z === 2,
...
check(
  firstTacklerTarget.x === secondTacklerTarget.x &&
    ...
    (firstTacklerTarget.z - tacklePhase.position.z) * attackDirection(tacklePhase.attackingTeam) > 0,
...
rollingTouchState.ball = {
  ...
  lastTouchedTeam: 0,
  passerId: null,
  kickerId: rollingTouchState.players.find((player) => player.team === 0)?.id ?? null,
  kickOrigin: { x: 0, z: 10 },
  bouncesRemaining: 0,
}
