...
export const clampZ = (z: number) => clamp(z, PITCH.deadBallLines.south + 1, PITCH.deadBallLines.north - 1)

export const getSlotIndex = (player: Player): number =>
  typeof player.slotIndex === 'number' ? clamp(player.slotIndex, 0, 14) : clamp((player.number - 1) % 15, 0, 14)
