import { type GameState, otherTeam, ROLES } from '../domain.ts'
import { getLineoutTarget, LINEOUT_MEMBER_VARIANTS } from '../formations.ts'
import { launchBall } from '../ball.ts'
import { startMaul } from './maul.ts'
import { clamp, distance, effectiveSkill } from '../math.ts'
import type { Random } from '../types.ts'

export const updateLineout = (state: GameState, deltaSeconds: number, random: Random) => {
  ...
  const throwingMembers = LINEOUT_MEMBER_VARIANTS[state.formations[phase.throwingTeam].lineoutMembers]
  const defendingMembers = LINEOUT_MEMBER_VARIANTS[state.formations[otherTeam(phase.throwingTeam)].lineoutMembers]
  ...
  const lineoutForwardsReady = state.players
    .filter((player) => {
      const members = player.team === phase.throwingTeam ? throwingMembers : defendingMembers
  ...
  if (phase.stage === 'forming') {
    ...
    if (hooker && state.ball.carrierId !== hooker.id && (hookerReady || phase.elapsed >= 1.0)) {
      state.ball = {
  ...
  if (phase.stage === 'ready') {
    ...
    const jumper =
      ...
      state.players.find(
        (player) => player.team === phase.throwingTeam && throwingMembers.includes((player.slotIndex ?? 0) + 1),
      )
    const defendingJumper =
      ...
      state.players.find(
        (player) => player.team !== phase.throwingTeam && defendingMembers.includes((player.slotIndex ?? 0) + 1),
      )
    ...
    const attackingScore = effectiveSkill(hooker, 'passing') * 0.4 + effectiveSkill(jumper, 'handling') * 0.6 + 0.18
    const defendingScore = defendingJumper
      ? effectiveSkill(defendingJumper, 'handling') * 0.55 + effectiveSkill(defendingJumper, 'decision') * 0.45
      : 0
    const turnoverProb = defendingJumper ? clamp(0.18 + (defendingScore - attackingScore) * 0.4, 0.05, 0.45) : 0
  ...
  if (state.ball.carrierId || state.ball.flight === null) {
    ...
    if (carrier?.team === phase.throwingTeam && random() < state.teams[phase.throwingTeam].tendencies.maul) {
