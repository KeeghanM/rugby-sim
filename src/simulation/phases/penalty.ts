...
import { launchBall, startGoalLineDropout } from '../ball.ts'
...
export const startPenalty = (
  ...
  const kicker =
    state.players.find((p) => p.team === awardedTeam && p.role === ROLES.FlyHalf) ??
    state.players.find((p) => p.team === awardedTeam)
  const targetTryLine = awardedTeam === 0 ? PITCH.tryLines.north : PITCH.tryLines.south
  ...
  const estimatedGoalChance = clamp(0.2 + kickSkill * 0.75 - anglePenalty - distancePenalty, 0.05, 0.95)
  ...
  const forwardPack = state.players.filter((p) => p.team === awardedTeam && isForward(p))
  const packWeight = forwardPack.reduce((sum, p) => sum + p.weight, 0)
  const maulTendency = state.teams[awardedTeam].tendencies.maul
  const packDominanceBonus = (packWeight > 880 ? 0.25 : 0) + (maulTendency > 0.5 ? 0.3 : 0)
  ...
  const choice = goalPreference + noise > touchPreference && estimatedGoalChance > 0.3 ? 'goal' : 'touch'
...
export const updatePenalty = (state: GameState, deltaSeconds: number, random: Random) => {
  ...
  const kicker =
    ...
    state.players.find((p) => p.team === phase.awardedTeam && p.role === ROLES.FlyHalf) ??
  ...
  const kickerReady = kicker !== undefined && distance(kicker.position, phase.position) <= 2.2
  const allPlayersOnside = state.players.every((player) =>
    player.team === phase.awardedTeam
      ? player.id === kicker?.id || (player.position.z - phase.position.z) * teamDir <= 0.2
  ...
  if (phase.stage === 'executing') {
    ...
    if (phase.choice === 'goal') {
      ...
      const targetTryLine = phase.awardedTeam === 0 ? PITCH.tryLines.north : PITCH.tryLines.south
      ...
      const isSuccess =
        !timedOut && random() < clamp(0.18 + kickSkill * 0.78 - anglePenalty - distancePenalty, 0.08, 0.94)

      phase.isSuccess = isSuccess
      kicker.stamina = clamp(kicker.stamina - 0.5, 0, 100)
      kicker.stats.totalKicks += 1

      const targetX = isSuccess ? (random() - 0.5) * 2.5 : (Math.sign(phase.position.x) || 1) * (6 + random() * 4)
      ...
      const peakHeight = isSuccess ? 7.0 + random() * 2.5 : 2.5 + random() * 2.5
    ...
    launchBall(state, kicker, { x: touchX * 1.05, z: touchZ }, 'kick', null, random, phase.position)
