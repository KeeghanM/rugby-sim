import { attackDirection, type GameState, PITCH, type Player } from '../../domain.ts'
...
export const attemptTackle = (state: GameState, random: Random) => {
  ...
  const carrierSkill = effectiveSkill(carrier, 'handling') * 0.7 + effectiveSkill(carrier, 'decision') * 0.3
  ...
  if (supportRunner && random() < carrierSkill * 0.26 * (1.15 - tacklerSkill * 0.45)) {
    carrier.stamina = Math.max(0, carrier.stamina - 0.5)
    tackler.tackleCooldown = 1.0
    launchBall(state, carrier, supportRunner.position, 'pass', supportRunner.id, random)
    return false
  }
  ...
  const massPenaltyFactor = weightDiff < 0 ? Math.max(0.2, 1.25 - tacklerSkill) : 1
  const massEffect = weightDiff * 0.0012 * massPenaltyFactor

  const tackleChance = clamp(
    0.78 + (tacklerSkill - carrierSkill) * 0.25 + massEffect - Math.max(0, carrierSpeed - 4.5) * 0.02,
    0.35,
    0.95,
  )
  ...
  const postContactMetres = clamp(
    1 + carrierSpeed * 0.5 + (carrier.weight - tackler.weight) * 0.01 + (carrierSkill - tacklerSkill) * 2,
    0.3,
    4,
  )
  ...
  const isAttackingInGoal =
    carrier.team === 0 ? carrier.position.z >= PITCH.tryLines.north : carrier.position.z <= PITCH.tryLines.south
  if (isAttackingInGoal) {
    scoreTry(state, carrier.team, random)
    return true
  }
  const isDefendingInGoal =
    carrier.team === 0 ? carrier.position.z <= PITCH.tryLines.south : carrier.position.z >= PITCH.tryLines.north
