import { otherTeam, PITCH, type Position, ROLES, type Team, type GameState } from '../domain.ts'
import { getScrumTarget, isForward } from '../formations.ts'
import { rerollTeamTactics } from '../teams/index.ts'
import { carryBall, launchBall } from '../ball.ts'
import { clamp, distance, effectiveSkill, overallSkill } from '../math.ts'
import type { Random } from '../types.ts'
import { resetContactPlayers } from '../contact.ts'

export const startScrum = (state: GameState, feedingTeam: Team, position: Position, random: Random = Math.random) => {
...
export const updateScrum = (state: GameState, deltaSeconds: number, random: Random) => {
  ...
  const forwardsReady = state.players
    ...
    .every((p) => {
      const target = getScrumTarget(
        ...
        state.activeShapePositions[p.team][p.team === phase.feedingTeam ? 'scrumAttack' : 'scrumDefence'],
      )
  ...
  if (phase.stage === 'set') {
    ...
    const packStrength = (team: Team) => {
      const forwards = state.players.filter((p) => p.team === team && isForward(p))
      if (forwards.length === 0) return 1
      const totalWeight = forwards.reduce((sum, p) => sum + p.weight, 0)
      const avgSkill =
        forwards.reduce((sum, p) => sum + effectiveSkill(p, 'tackling') * 0.6 + overallSkill(p) * 0.4, 0) /
        forwards.length
      const avgFatigue = forwards.reduce((sum, p) => sum + (0.5 + (p.stamina / 100) * 0.5), 0) / forwards.length
    ...
    const strengthDifference = (defendingPackStrength - feedingPackStrength) / Math.max(1, averageStrength)
    // Feed remains an advantage, but pack weight and technique decide turnovers.
    const turnoverRoll = random()
    const turnoverThreshold = clamp(0.12 + strengthDifference * 0.45, 0.03, 0.45)
    phase.winningTeam = turnoverRoll < turnoverThreshold ? otherTeam(phase.feedingTeam) : phase.feedingTeam
  ...
  if (phase.stage === 'channeling') {
    ...
    const nine = state.players.find((p) => p.team === winningTeam && p.role === ROLES.ScrumHalf)
    const eight = state.players.find((p) => p.team === winningTeam && p.role === ROLES.NumberEight)
    ...
    if (eight && random() < 0.25) {
      ...
      const ten = state.players.find((p) => p.team === winningTeam && p.role === ROLES.FlyHalf)
      if (ten) {
