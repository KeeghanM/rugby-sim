import type { GameState, MatchInput } from './domain.ts'
import { computeCommands } from './decisions/index.ts'
import { createGame, createMatchInput } from './create-game.ts'
import { createMatchResult } from './match-result.ts'
import { advanceDefensiveLine, applyCommands } from './movement/index.ts'
import type { Random } from './types.ts'

export const SIMULATION_STEP_SECONDS = 0.05

export { createGame, createMatchInput } from './create-game.ts'
export { createMatchResult, SIMULATION_VERSION } from './match-result.ts'
export { computeCommands } from './decisions/index.ts'
export { applyCommands } from './movement/index.ts'
...
export const updateGame = (state: GameState, deltaSeconds: number, random: Random = Math.random) => {
...
export type MonteCarloSummary = {
  ...
  contestWinRate: Record<'ruck' | 'maul' | 'scrum' | 'lineout', [number, number]>
}
...
export const simulateMonteCarlo = (
  ...
  } as Record<'ruck' | 'maul' | 'scrum' | 'lineout', [[number, number], [number, number]]>
  let draws = 0
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    for (const team of [0, 1] as const) {
      ...
      tries[team] += players.reduce((total, player) => total + player.stats.triesScored, 0)
      knockOns[team] += players.reduce((total, player) => total + player.stats.knockOns, 0)
      tacklesMade[team] += players.reduce((total, player) => total + player.stats.tacklesMade, 0)
      tacklesMissed[team] += players.reduce((total, player) => total + player.stats.tacklesMissed, 0)
      const teamStats = result.teamStats[team]
      for (const contest of Object.keys(contests) as (keyof typeof contests)[]) {
        const prefix = `${contest}s` as 'rucks' | 'mauls' | 'scrums' | 'lineouts'
  ...
  return {
    ...
    tries,
    knockOns,
    tackleCompletion: [ratio(tacklesMade[0], tacklesMissed[0]), ratio(tacklesMade[1], tacklesMissed[1])],
    contestWinRate,
  }
}
