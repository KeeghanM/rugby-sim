...
export const loadPreset = (teams: MatchConfig, team: Team, presetKey: string) => {
...
export const createMatchConfig = (source: MatchConfig = TEAMS): MatchConfig => ({
  0: cloneTeam(source[0]),
  1: cloneTeam(source[1]),
})

export type TeamStatsInput = Partial<
  Pick<TeamDefinition, 'name' | 'color' | 'lineSpeed' | 'speedMultiplier' | 'weightMultiplier'>
> & {
...
export const setStats = (teams: MatchConfig, team: Team, stats: TeamStatsInput) => {
  ...
  if (stats.lineSpeed !== undefined) target.lineSpeed = clamp(requireFinite(stats.lineSpeed), 1, 8)
  if (stats.speedMultiplier !== undefined) {
    target.speedMultiplier = clamp(requireFinite(stats.speedMultiplier), 0.7, 1.3)
  }
  if (stats.weightMultiplier !== undefined) {
    target.weightMultiplier = clamp(requireFinite(stats.weightMultiplier), 0.7, 1.3)
  }
  if (stats.skills) {
    for (const skill of Object.keys(stats.skills) as (keyof PlayerSkills)[]) {
      const value = stats.skills[skill]
      if (value !== undefined) target.defaultSkills[skill] = clamp(requireFinite(value), 0, 1)
    }
  }
...
export const setTactics = (teams: MatchConfig, team: Team, tactics: TeamTacticsInput) => {
  const target = teams[team]
  const carry = tactics.carry === undefined ? target.tendencies.carry : Math.max(0, requireFinite(tactics.carry))
  const pass = tactics.pass === undefined ? target.tendencies.pass : Math.max(0, requireFinite(tactics.pass))
  const kick = tactics.kick === undefined ? target.tendencies.kick : Math.max(0, requireFinite(tactics.kick))
  const total = carry + pass + kick
  if (total === 0) throw new RangeError('Carry, pass, and kick cannot all be zero')
  if (tactics.formations) {
    Object.assign(target.formations, tactics.formations)
  }
  if (tactics.customFormations) {
    for (const [context, positions] of Object.entries(tactics.customFormations) as [
      FormationContext,
      Position[] | null,
    ][]) {
      if (positions) {
  ...
  if (tactics.tacticalShapes) {
    ...
    for (const [context, shapes] of Object.entries(tactics.tacticalShapes) as [FormationContext, TacticalShape[]][]) {
  ...
  if (tactics.formationVariation !== undefined) {
    target.formationVariation = clamp(requireFinite(tactics.formationVariation), 0, 1)
  }
  if (tactics.maul !== undefined) target.tendencies.maul = clamp(requireFinite(tactics.maul), 0, 1)
