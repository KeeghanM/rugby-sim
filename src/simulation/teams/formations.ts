...
const pickVariant = <T>(defaultVal: T, all: readonly T[], variation: number, random: Random): T => {
  if (random() >= variation) return defaultVal
  return all[Math.floor(random() * all.length)]
}

const pickWeightedShape = (shapes: readonly TacticalShape[] | undefined, random: Random) => {
...
const rollContext = <Context extends FormationContext>(
  ...
  const formation =
    shape?.preset && variants.includes(shape.preset as ActiveTeamFormations[Context])
      ? (shape.preset as ActiveTeamFormations[Context])
      : pickVariant(configured, variants, definition.formationVariation, random)
  return {
    formation,
...
export const rollTeamTactics = (
  ...
  const kickoffAttack = rollContext('kickoffAttack', definition, KICKOFF_ATTACK_VARIANTS, random)
  const kickoffDefence = rollContext('kickoffDefence', definition, KICKOFF_DEFENCE_VARIANTS, random)
  const openAttack = rollContext('openAttack', definition, OPEN_ATTACK_VARIANTS, random)
  const openDefence = rollContext('openDefence', definition, OPEN_DEFENCE_VARIANTS_LIST, random)
  const scrumAttack = rollContext('scrumAttack', definition, SCRUM_ATTACK_VARIANTS, random)
  const scrumDefence = rollContext('scrumDefence', definition, SCRUM_DEFENCE_VARIANTS, random)
  ...
  return {
    formations: {
      ...
      lineoutMembers: pickVariant(def.lineoutMembers, LINEOUT_MEMBERS_LIST, variation, random),
      lineoutNonParticipants: pickVariant(def.lineoutNonParticipants, LINEOUT_NON_PARTICIPANTS_LIST, variation, random),
