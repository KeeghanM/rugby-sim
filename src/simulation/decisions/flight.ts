import { attackDirection, PITCH, ROLES, type GameState, type Player, type Position } from '../domain.ts'
...
const predictedLanding = (state: GameState): Position => {
  ...
  const time = Math.max(
    0,
    (ball.velocity.y + Math.sqrt(ball.velocity.y ** 2 + 2 * GRAVITY * ball.position.y)) / GRAVITY,
  )
...
export const computeFlightCommands = (state: GameState, players: Player[]) => {
  ...
  const contestableKick =
    state.ball.flight === 'kick' || state.ball.flight === 'kickoff' || state.ball.flight === 'rolling'
  ...
  const eligibleChasers = new Set(
    players
      .filter((player) => {
        ...
        if (isKickoff) {
          return player.role !== ROLES.FullBack && player.role !== ROLES.InsideCentre
        }
        return true
      })
  ...
  const receivingCatchers = new Set(
    players
      ...
      .map((player) => {
        ...
        const priorityRole =
          isForward(player) || player.role === ROLES.InsideCentre || player.role === ROLES.OutsideCentre
  ...
  return players.map((player) => {
    ...
    if (eligibleChasers.has(player.id)) {
      const chaseTarget = isKickoff ? { x: player.laneX, z: landing.z } : landing
