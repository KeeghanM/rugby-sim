import { attackDirection, otherTeam, ROLES, type GameState, type Player, type Position } from '../domain.ts'
...
export const nearestOpponentDistance = (players: Player[], player: Player) =>
  players.reduce(
    (nearest, other) =>
      other.team === player.team ? nearest : Math.min(nearest, distance(player.position, other.position)),
    Infinity,
  )

export const hasBrokenLine = (state: GameState, players: Player[], carrier: Player) => {
  ...
  const frontlineAhead = players.filter(
    (p) => p.team === defendingTeam && p.role !== ROLES.FullBack && (p.position.z - carrier.position.z) * direction > 0,
  )
  if (frontlineAhead.length <= 1) return true
  const frontlineBeaten = players.filter(
    (p) => p.team === defendingTeam && p.role !== ROLES.FullBack && (carrier.position.z - p.position.z) * direction > 0,
  )
  return frontlineBeaten.length >= 2
}

export const selectSupportRunners = (players: Player[], carrier: Player, lineBroken: boolean) => {
  const direction = attackDirection(carrier.team)
  ...
  return players
    ...
    .map((player) => {
      const preferred = central
        ? isForward(player) || player.role === ROLES.FlyHalf || player.role === ROLES.InsideCentre
      ...
      return {
        ...
        priority:
          ...
          Math.max(0, (player.position.z - carrier.position.z) * direction) * 1.5 +
  ...
  players
    .filter(
      (player) =>
        ...
        (!flowDirection || (player.position.x - carrier.position.x) * flowDirection >= 2) &&
        (player.position.z - carrier.position.z) * attackDirection(carrier.team) <= 0.5 &&
        (player.position.z - carrier.position.z) * attackDirection(carrier.team) >= (preferredRoles ? -12 : -5) &&
    ...
    .sort(
      ...
      (a, b) => a.depth - b.depth || a.lateralGap - b.lateralGap || b.space - a.space || a.gap - b.gap,
    )[0]?.player

export const clearanceTarget = (player: Player, random: Random): Position => {
  const direction = attackDirection(player.team)
  const isBackThreeOrTen = player.role === ROLES.FullBack || player.role === ROLES.FlyHalf || player.role === ROLES.Wing
  const kickSkill = effectiveSkill(player, 'kicking')
  const targetTryLine = direction === 1 ? PITCH.tryLines.north : PITCH.tryLines.south
  const maxSafeDistance = Math.max(14, Math.abs(targetTryLine - player.position.z) - 8)
  ...
  const finalDistance = isMiskickOvercooked ? desiredDistance + 24 : desiredDistance
  const side = Math.abs(player.position.x) > 6 ? Math.sign(player.position.x) : random() < 0.5 ? -1 : 1
  return {
    x: side * (PITCH.touchLines.right + 6),
    z: clamp(player.position.z + direction * finalDistance, PITCH.tryLines.south - 5, PITCH.tryLines.north + 5),
  }
}
