import { attackDirection, otherTeam, ROLES, type GameState, type Player } from '../domain.ts'
import { getOpenPlayTarget } from '../formations.ts'
...
export const getOpenPlayCommands = (
  ...
  const defendersAhead = players.filter(
    (player) => player.team === defendingTeam && (player.position.z - carrier.position.z) * ballDirection > 0,
  )

  const fullback = players.find((player) => player.team === defendingTeam && player.role === ROLES.FullBack)
  const lineTackler = players
    .filter((player) => player.team === defendingTeam && player.role !== ROLES.FullBack)
  ...
  return players.map((player) => {
    ...
    const target = getOpenPlayTarget(
      ...
      state.activeShapePositions[player.team][attacking ? 'openAttack' : 'openDefence'],
    )
    ...
    if (supportIndex >= 0) {
      ...
      return command(
        ...
        false,
        lineBroken || distance(player.position, carrier.position) > 8 ? 'sprint' : 'run',
      )
    }
    ...
    if (isAheadOfBall) {
      ...
      if (aheadDistance <= 3.5 && isCarrierRunningForward) {
        return command(player, { x: player.laneX, z: player.position.z }, 'await-carrier', false, 'stand')
      }
      const clearX =
        lateralDist < 8
          ? clamp(player.position.x + (player.position.x >= carrier.position.x ? 6 : -6), -32, 32)
          : player.laneX
        ...
        lateralDist > 16
          ...
          : aheadDistance > 14
            ...
            : aheadDistance > 6
              ? 'run'
              : 'jog'
      return command(player, { x: clearX, z: targetZ }, 'offside-recovery', false, effort)
    }

    if (!attacking) {
      const distToCarrier = distance(player.position, carrier.position)
      const isFullback = player.role === ROLES.FullBack
      const isAheadOfCarrier = (player.position.z - carrier.position.z) * ballDirection > 0
      const isLastDefenderAhead = isAheadOfCarrier && (defendersAhead.length <= 1 || isFullback)
    ...
    return command(
      ...
      false,
      attacking ? (formationGap > 12 ? 'sprint' : formationGap > 2 ? 'run' : 'jog') : 'run',
    )
  })
}
