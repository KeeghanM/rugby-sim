import { attackDirection, PITCH, ROLES, type GameState, type Player } from '../domain.ts'
import { getKickoffTarget, getLineoutTarget, getRuckTarget, getScrumTarget } from '../formations.ts'
...
export const getLineoutCommands = (state: GameState, players: Player[]) => {
  ...
  return players.map((player) => {
    ...
    return command(player, target, `lineout-${phase.stage}`, false, gap > 8 ? 'sprint' : gap > 1.5 ? 'run' : 'stand')
  })
}
...
export const getScrumCommands = (state: GameState, players: Player[]) => {
  ...
  return players.map((player) => {
    ...
    const target = getScrumTarget(
      ...
      state.activeShapePositions[player.team][player.team === phase.feedingTeam ? 'scrumAttack' : 'scrumDefence'],
    )
    const gap = distance(player.position, target)
    const effort =
      // Set and channeling packs remain bound while contest resolves in phase logic.
      phase.stage === 'set' || phase.stage === 'channeling' ? 'stand' : gap > 8 ? 'sprint' : gap > 1.5 ? 'run' : 'stand'
...
export const getMaulCommands = (state: GameState, players: Player[]) => {
  ...
  return players.map((player) => {
    const group = attackers.has(player.id) ? phase.attackers : defenders.has(player.id) ? phase.defenders : null
    if (group) {
      ...
      return command(
        player,
        {
          ...
          x: clamp(phase.position.x + (rank - (group.length - 1) / 2) * 0.7, -33, 33),
    ...
    return command(
      ...
      {
        x: player.laneX,
        z: phase.position.z + direction * (player.team === phase.attackingTeam ? -8 : 8),
      },
      'maul-shape',
      false,
      'run',
...
export const getConversionCommands = (state: GameState, players: Player[]) => {
  ...
  return players.map((player) => {
    const isKicker =
      player.team === phase.kickingTeam && (player.id === phase.kickerId || player.role === ROLES.FlyHalf)
    ...
    if (player.team === phase.kickingTeam) {
      ...
      const gap = distance(player.position, target)
      return command(player, target, 'conversion-support', false, gap > 2 ? 'run' : 'stand')
    }
    const target = {
    ...
    const gap = distance(player.position, target)
    return command(player, target, 'conversion-defence', false, gap > 2 ? 'run' : 'stand')
  })
}
    ...
    if (player.team === phase.awardedTeam) {
      ...
      return command(
        player,
        {
          ...
          z: clamp(phase.position.z - teamDir * (2 + (slotIndex % 3) * 1.5), -55, 55),
        },
        'penalty-attack',
        false,
        'run',
...
export const getRuckCommands = (state: GameState, players: Player[]) => {
  ...
  return players.map((player) => {
    ...
    if (player.id === phase.tackledPlayerId) {
      const frozen = command(player, player.position, 'ruck-contact-frozen', true, 'stand')
      frozen.freeze = true
      return frozen
    }
    ...
    const target = getRuckTarget(player, phase.position, phase.attackingTeam, attackers, defenders)
    const direction = attackDirection(phase.attackingTeam)
    const offsideZ =
      player.team === phase.attackingTeam ? phase.position.z - direction * 0.5 : phase.position.z + direction * 0.5
