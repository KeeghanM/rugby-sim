...
import { choosePassTarget, clearanceTarget, command, hasBrokenLine, nearestOpponentDistance } from './utils.ts'
...
export const chooseCarrierCommand = (
  ...
  if (carrier.pendingBallAction) {
    ...
    return command(
      carrier,
      isKick ? carrier.position : { x: carrier.position.x, z: carrier.position.z + direction * 4 },
  ...
  const flowDirection = carrier.position.x <= -25 ? 1 : carrier.position.x >= 25 ? -1 : state.attackFlow[carrier.team]
  if (lineBroken) {
  ...
  if (carrier.decisionForSeconds > 0) {
    const continuing = command(carrier, carrier.intentTarget, 'carrier', false, 'run')
  ...
  result.decisionForSeconds = 0.25 + (1 - effectiveSkill(carrier, 'decision')) * 0.65
  ...
  const nearbyTeammates = players.filter(
    (p) => p.team === carrier.team && p.id !== carrier.id && distance(p.position, carrier.position) <= 15,
  )
  const isStranded = nearbyTeammates.length === 0 && defendersAhead.length > 0
  const recognisesClearance = random() >= (1 - effectiveSkill(carrier, 'decision')) * 0.18
  ...
  const targetTryLine = carrier.team === 0 ? PITCH.tryLines.north : PITCH.tryLines.south
  ...
  if (canAttemptDropGoal) {
    ...
    const proximityScore = (1 - distToGoalLine / 40) * (1 - angleFromPosts / 22)
    const spaceBonus = Math.min(1.5, nearestDefDist / 6)
    const dropGoalChance = 0.28 * proximityScore * spaceBonus * state.teams[carrier.team].tendencies.kick * 3
  ...
  const roll = random()
  const passTarget = choosePassTarget(players, carrier, undefined, isErratic ? undefined : flowDirection)
  ...
  if (canKick) {
    if (insideOwnTwentyTwo(carrier.team, carrier.position.z) || carrier.role === ROLES.FullBack) {
      result.ballAction = {
        kind: 'kick',
        target: clearanceTarget(carrier, random),
        flight: 'kick',
