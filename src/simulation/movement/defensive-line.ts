...
export const advanceDefensiveLine = (state: GameState, deltaSeconds: number) => {
  ...
  if (state.phase.kind === 'ruck') {
    ...
    state.defensiveLineZ[otherTeam(state.phase.attackingTeam)] = state.phase.position.z + direction * 0.5
  ...
  const defenders = state.players.filter((player) => player.team === defendingTeam)
  const defensiveSkill =
    defenders.reduce(
      (total, player) => total + effectiveSkill(player, 'tackling') * 0.6 + effectiveSkill(player, 'decision') * 0.4,
      0,
    ) / Math.max(1, defenders.length)
  // Team line speed is tempered by average tackling and decision quality.
  const advanced =
    currentLine - direction * state.teams[defendingTeam].lineSpeed * (0.65 + defensiveSkill * 0.5) * deltaSeconds
