...
export const resolvePreparedAction = (state: GameState, carrier: Player, deltaSeconds: number, random: Random) => {
  ...
  if (pending.kind === 'pass') {
    const receiver = state.players.find(
      (player) => player.id === pending.receiverId && player.team === carrier.team && player.ruckRecoverySeconds === 0,
    )
    if (!receiver) return
    const passDepth = (receiver.position.z - carrier.position.z) * attackDirection(carrier.team)
  ...
  if (chargingDefender) {
    ...
    const isChargedDown = random() < (dist < 1.4 ? 0.42 : 0.2) * (1.25 - effectiveSkill(carrier, 'kicking') * 0.75)
