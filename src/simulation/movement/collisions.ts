import { ROLES, type GameState, type Player, type Position } from '../domain.ts'
import { isForward } from '../formations.ts'
import { distance } from '../math.ts'

export const separatedVelocity = (state: GameState, player: Player, velocity: Position): Position => {
  const phase = state.phase
  const isRuckRunner =
    phase.kind === 'ruck' &&
    (player.id === phase.tacklerId ||
      (player.role === ROLES.ScrumHalf && player.team === (phase.winningTeam ?? phase.attackingTeam)))
  if (
  ...
  for (const other of state.players) {
    ...
    const isOtherMaulBound =
      maulPhase !== null && (maulPhase.attackers.includes(other.id) || maulPhase.defenders.includes(other.id))
