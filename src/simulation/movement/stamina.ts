...
export const updateStamina = (state: GameState, player: Player, next: PlayerCommand, deltaSeconds: number) => {
  ...
  const effortMod = next.effort === 'sprint' ? 2.1 : next.effort === 'run' ? 1.0 : next.effort === 'jog' ? 0.4 : 0
  ...
  const netRate = atTarget || next.effort === 'stand' ? recoveryRate : -baseDrainRate * effortMod
