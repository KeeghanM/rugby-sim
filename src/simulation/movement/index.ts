import { type GameState, PITCH } from '../domain.ts'
...
export const applyCommands = (
  ...
  const nextMotion = commands.map((next) => {
    ...
    if (!player) throw new Error(`Command references unknown player ${next.playerId}`)
    ...
    player.breakawaySeconds = Math.max(0, player.breakawaySeconds - deltaSeconds)
    player.hardLineForSeconds = next.startHardLine ? 1.5 : Math.max(0, player.hardLineForSeconds - deltaSeconds)
    player.ruckRecoverySeconds = Math.max(0, player.ruckRecoverySeconds - deltaSeconds * 6)
    ...
    const desired = separatedVelocity(state, player, desiredVelocity(player, player.intentTarget, next.effort))
  ...
  for (const motion of nextMotion) {
    const distTraveled = Math.hypot(motion.velocity.x * deltaSeconds, motion.velocity.z * deltaSeconds)
